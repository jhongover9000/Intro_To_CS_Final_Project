...
class Player(Entity):
    ...
    def display(self):
        ...
        noFill()
        strokeWeight(3)
        #rect(self.hitRangex[0],self.hitRangey[0],(self.hitRangex[-1]-self.hitRangex[0]),(self.hitRangey[-1]-self.hitRangey[0])) #x,y,w,h
...
class Enemy(Entity):
    ...
    def attackPlayer(self):
        if self.type == "melee":
            if self.action != "dead": #Makes sure Enemy can't attack while dead
                ...
                '''
                if (self.x + self.w/2.4 - g.middlex > g.kirito.x) and (self.x + self.w/2.4 - g.middlex - (g.kirito.x + 100) < (self.hitRangex[-1]-self.hitRangex[0])): #If enemy x greater and distance between enemy x and kirito x is less than width of enemy
                    print("LEFT TRUE", self.action, self.lastAction)
                    self.vx = 0
                    self.action = "attack"
                ...
                elif (self.x + self.w/2.4 - g.middlex < g.kirito.x) and (self.x + self.w/2.4 - g.middlex - (g.kirito.x + 100) < (self.hitRangex[-1]-self.hitRangex[0])):
                    print("RIGHT TRUE", self.action, self.lastAction)
                    self.vx = 0 
                    self.action = "attack"
                    self.status = "attacking"
                    self.lastAction = "attack"
    ...
    #    del g.enemies[self.index] #Removes itself from the List
    ###############################################################################################################################
